...
function renderStep() {
  ...
  if (step === 1) {
    ...
    content.innerHTML = `<p class="section-kicker">${selectedCase.uncertainty_category} uncertainty</p><h1>The model keeps ${conformal.length} plausible labels in view.</h1><div class="panel"><h2>Empirical inclusion set</h2><p>${conformal.map((label) => `<span class="tag">${titleCase(label)}</span>`).join(" ")}</p><p>This set is an uncertainty boundary, not a list of confirmed diagnoses. Larger sets indicate ambiguity and increase the need for review or confirmatory testing.</p></div>${callout("The exact and pragmatic policies are reported separately. No prospective coverage guarantee is claimed.")}`;
  } else {
